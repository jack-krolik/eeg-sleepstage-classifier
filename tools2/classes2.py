...
class NightBasedCrossValidator:
    ...
    #         class_ratios = class_counts.float() / total
    #         class_ratios_per_night.append(class_ratios)
        
    #     # Get minimum ratios across nights with a small buffer
    #     min_ratios = torch.stack(class_ratios_per_night).min(dim=0)[0]
    #     # Add safety buffer of 0.8 to minimum ratios
    #     min_ratios = min_ratios * 0.8
    #     min_floor = 0.01  # Minimum required ratio for any class
    #     min_ratios = torch.maximum(min_ratios, torch.tensor(min_floor))
